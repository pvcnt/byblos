...
public final class LeftValueAxis extends ValueAxis {
    public LeftValueAxis(PlotDef plotDef, Styles styles, double min, double max) {
    ...
    public void draw(Graphics2D g, int x1, int y1, int x2, int y2) {
        ...
        if (null != label) {
            drawLabel(label, g, x1, y1, x1 + labelHeight, y2);
        }
    }
    ...
    private void drawNormal(List<ValueTick> ticks, Graphics2D g, int x1, int y1, int x2, int y2) {
        ...
        for (var tick: ticks) {
            ...
            if (plotDef.showTickLabels()) {
                var txt = Text.right(tick.getLabel(), ChartSettings.smallFont, style);
    ...
    private void drawWithOffset(List<ValueTick> ticks, Graphics2D g, int x1, int y1, int x2, int y2) {
        ...
        var offsetTxt = Text.left(offsetStr, ChartSettings.smallFont, style);
        ...
        ticks.stream().skip(1).forEach(tick -> {
            ...
            if (plotDef.showTickLabels()) {
                ...
                var txt = Text.right(label, ChartSettings.smallFont, style);
                var txtH = ChartSettings.smallFontDims.height();
                var ty = py - txtH / 2;
                if (ty + txtH < otop) {
                    txt.draw(g, x1, ty, x2 - tickMarkLength - 1, ty + txtH);
