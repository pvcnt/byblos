...
import dev.byblos.model.ConstantExpr;
...
public class GraphConfigFactoryTest {
    ...
    void simpleExpr() {
        ...
        Assertions.assertThat(lowerBound(cfg)).isEqualTo(AutoStyleBound.INSTANCE);
    ...
    void lowerBoundExplicit() {
        ...
        Assertions.assertThat(lowerBound(cfg)).isEqualTo(new ExplicitBound(0d));
    }

    @Test
    void lowerBoundAutoData() {
        var cfg = factory.toGraphConfig(newGraphRequest(Map.of("q", "42,:const", "l", "auto-data")));
        Assertions.assertThat(lowerBound(cfg)).isEqualTo(AutoDataBound.INSTANCE);
    }
